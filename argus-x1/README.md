# ARGUS X-1

ARGUS X-1, with only a $300 budget, exceeded my expectations and performed exceptionally well in its first baseline performance test. It was tested under optimal indoor conditions with fixed exposure at 150, a high-contrast red LED target under green ambient lighting, without a Kalman filter. Three deliberate reacquisitions were successfully made and the system operated continuously.

### Performance Metrics

| Metric                         | Value                          |
|--------------------------------|--------------------------------|
| Closed-loop update rate        | 60.0 Hz                        |
| Jitter (1σ)                    | 1.93 ms                        |
| Tracking uptime                | 96.5% over 90.85 s             |
| RMS error (X/Y)                | 12.76 px / 11.32 px            |
| Mean bias (X/Y)                | -0.011 px / +0.390 px          |
| Occlusion recoveries           | 0.314 s, 1.482 s, 1.253 s      |
| 95th percentile absolute error | 27.46 px / 22.63 px            |
| Median absolute error          | 4.93 px / 5.28 px              |
| Time to first acquisition      | 0.313 s                        |
| Settle time                    | 0.117 s                        |
